var rule = {
    ...
    play_parse: true,
    play_json: [],
    // 重要：lazy 会经 $js.toString 后作为“语句”被 eval，函数体里严禁顶层 return
    lazy: $js.toString(() => {
        let host = 'https://www.gmp4.com';
        let m3u8 = '';
        if (typeof input === 'string' && input.indexOf('http') === 0) {
            // 已是直链（qiwei_direct 二级已解析好）：直接拿它播放
            m3u8 = input.split('$').pop();
        } else {
            // 否则 input 是播放页路径，去抓直链
        ...
        // 关键：把 input 重赋值成对象 + parse:0 => 播放器直链播放，不再外部解析
        input = {
            url: m3u8,
            parse: 0,
            jx: 0,
