...
var rule = {
    ...
    lazy: $js.toString(() => {
        try {
            ...
            var html = fetch(u, fetch_params);
            var i = html.indexOf('var player_aaaa=');
            if (i < 0) {
                // 页面无内嵌直链, 交给外部解析器去解析该页面
            ...
            var obj = JSON.parse(html.slice(s, e));
            var url = (obj && obj.url) ? obj.url : '';
            if (url && url.indexOf('http') >= 0 && url.indexOf('.') > 0) {
                // obj.url 即为真实 m3u8/mp4 直链, 直接可播, 无需解析
                input = { url: url, parse: 0, jx: 0, header: { 'User-Agent': '' } };
            } else {
                // 直链为空(可能需鉴权/解析), 回退页面交给外部解析器
                input = { url: u, header: { 'User-Agent': '' }, parse: 1, jx: 1 };
            }
        } catch (ex) {
            input = { url: 'https://www.gmp4.com' + String(input).replace(host, ''), parse: 1, jx: 1 };
        }
    }),
    //------------------------------------------------------------------
