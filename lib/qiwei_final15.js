var rule = {
    ...
    lazy: $js.toString(() => {
        ...
        input = { url: m3u8, parse: 0, jx: 0, header: { 'User-Agent': ua, 'Referer': host + '/' } };
    }),
    二级: $js.toString(() => {
        VOD = {};
        try {
            ...
            let lines = [];
            // 第1条：已验证可播的真实直链，用于一次判定播放层
            lines.push('【测试】判定$https://svip.xgplay17.com/2026-08-18/36069_iAU3yMNcnbwzBzJtMU/index.m3u8');
            // 真实剧集：把前N集直接解析成真实直链(控制耗时防超时)，其余走lazy
            let n = eps.length > 10 ? 10 : eps.length;
            for (let k = 0; k < n; k++) {
                let u = '';
                try {
