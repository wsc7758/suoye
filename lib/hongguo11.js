...
var rule = {
    ...
    一级: "js:(" + function () {
        ...
        try {
            ...
            var html = '';
            try { html = String(fetch(myUrl, fetch_params)); } catch (e_fetch) { setResult([{ title: 'fetch异常:' + (e_fetch && e_fetch.message), img: '', desc: '', url: '/player/7553892968508181529' }]); return; }
            if (!html || html.length < 200) { setResult([{ title: '空页(size:' + html.length + ')', img: '', desc: '', url: '/player/7553892968508181529' }]); return; }
            var arr = [];
